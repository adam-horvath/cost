    ...
    return this.props.dashboard.items ? (
      <div className="dashboard">
        <div className="balance">
          Egyenleg: {Util.getMoneyString(this.props.dashboard.balance)}
        </div>
        <div className="add-item-container">
          <div
            className="add-container add-cost-container"
            onClick={() => this.showItemModal({ isCost: true }, "ADD")}
          >
          ...
          <div
            ...
            onClick={() => this.showItemModal({ isCost: false }, "ADD")}
          >
            <div className="add-label add-income-label">Bevétel</div>
        ...
        <div className="date-picker-container">
          ...
          <DatePicker
            selected={moment.tz(this.state.date, Constants.TIME_ZONE)}
            onChange={this.onDateChange}
            locale="hu"
            dropdownMode={"select"}
            todayButton={"Mai nap"}
            isClearable={false}
            minDate={moment.tz(new Date("2015-09-30"), Constants.TIME_ZONE)}
          ...
          {this.props.dashboard.items.map(item => (
            <div
              ...
              onClick={() =>
                this.showItemModal(
                  {
                    id: item._id,
                    isCost: item.category_type.toLowerCase() === "cost",
                    amount: item.amount,
                    selectedMenuItem:
                      item.category_type.toLowerCase() === "cost"
                        ? Constants.COST_CATEGORIES.indexOf(
                            Util.getHungarianCategory(item.category)
                          )
            ...
            >
              ...
              <div className="item-texts">
                <div className="amount">{Util.getMoneyString(item.amount)}</div>
