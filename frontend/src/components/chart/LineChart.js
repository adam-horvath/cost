...
class LineChart extends Component {
  ...
  draw = () => {
    ...
    const node = d3.select(this.domNode);

    let isHorizontalLabels = false;
    this.width =
      parseInt(node.style("width"), 10) - this.margin.left - this.margin.right;

    this.margin.bottom =
      Util.getMaxTextWidth(this.labels, "normal 12px VarelaRound") + 10;
      ...
      if (this.props.chart.hasOwnProperty(key)) {
        minValue = Math.min(
          ...
          ...this.props.chart[key].map(
            d =>
              this.props.isSum && key === "BALANCE" ? d.sumAmount : d.amount
          )
        );
        maxValue = Math.max(
          maxValue,
        ...
        d3
          ...
          .text(
            Util.getHungarianCategory(key) +
              (this.props.isSum && key === "BALANCE" ? " (\u03A3)" : "")
          )
    ...
    const line = d3
      ...
      .y(
        d =>
          this.props.isSum && d.sumAmount ? mapY(d.sumAmount) : mapY(d.amount)
      )
    ...
    for (let key in this.props.chart) {
      if (this.props.chart.hasOwnProperty(key)) {
        console.log(this.props.chart[key]);
        g
          .append("path")
          .attr("class", "line")
    ...
    d3
      ...
      .attr(
        ...
        "translate(" +
          (translateX +
            (this.tooltipWidth -
              Util.getTextWidth(tooltipText, "normal 16px VarelaRound")) /
              2) +
          ", " +
