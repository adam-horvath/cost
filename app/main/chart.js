...
let getChartData = (req, res) => {
    ...
    auth.getUserAndGroup(auth.getToken(req.headers))
        .then((authResult) => {
            ...
            const now = new Date();
            if (!maxDate.year || maxDate.year > now.getFullYear() || maxDate.year == now.getFullYear() && maxDate.month > now.getMonth()) {
                maxDate = {
            ...
            let tasksToGo = months.length * (categories.length + categoryTypes.length);
