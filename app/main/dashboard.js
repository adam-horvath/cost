...
const getMainDashboard = async (req, res) => {
    const { user, group } = await auth.getUserAndGroup(
...
const getStats = async (req, res) => {
    ...
    for (const item of itemsOfMonth) {
        if (item.category_type === "INCOME") {
            ...
            if (item.category !== "HOUSE") totalCostWithoutHouse += item.amount;
            if (!["HOUSE", "INVESTMENT", "PHONE"].includes(item.category))
    ...
    const dailyAverageWithoutHouse = totalCostWithoutHouse / quotient;
    const dailyAverageOfGeneralCost = generalCost / quotient;
    const costOnce = Math.max(costOnceOfPreviousMonth, costOnceOfThisMonth);
    const expectedCost = isThisMonth(year, month)
        ? dailyAverageOfGeneralCost * daysInMonth + costOnce
        : totalCost;
...
const deleteItem = async (req, res) => {
    ...
    if (!itemId) {
        return res
            .status(400)
            .send({ success: false, msg: "No item provided." });
    }
    const { group } = await auth.getUserAndGroup(auth.getToken(req.headers));
...
const updateItem = async (req, res) => {
    ...
    if (
        newDate.getFullYear() !== item.year ||
        newDate.getMonth() !== item.month
    ) {
        return res
            .status(400)
            .send({
    ...
    if (isModifiedAmount) {
        ...
        return res
            .status(200)
            .send({ success: true, msg: "Sikeres mentés új összeggel." });
    }
    return res
        .status(200)
        .send({
...
const addItem = async (req, res) => {
    ...
    const factor = item.category_type === "INCOME" ? 1 : -1;
    const change = item.amount * factor;
    const balances = await getBalancesFromMonth(
