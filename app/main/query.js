    ...
    auth
        ...
        .then(result => {
            ...
            if (collection.toLowerCase() == "balance") {
                ...
                Balance.findOne(
                    ...
                    (err, balance) => {
                        if (err || !balance) {
                            return res
                                .status(400)
                                .send({ success: false, msg: "Nincs adat." });
                        }
                        return res
                            .status(200)
                            .send({ success: true, value: balance.amount });
                    }
                );
                ...
                const exclude_category = req.body.exclude_category;
                ...
                if (description_like == true && !description) {
                    return res
                        .status(400)
                        .send({ success: false, msg: "Wrong parameters." });
                }
                let query = {
...
let queryList = (req, res) => {
    auth
        ...
        .then(result => {
            ...
            let minDate = min_date ? new Date(min_date) : new Date(2000, 1, 1);
            let maxDate = max_date ? new Date(max_date) : new Date(2100, 11, 30);
            query.date = {
                $gte: minDate,
                $lte: maxDate
            };
