...
const archive = async (req, res) => {
    ...
    return res
        .status(200)
        .send({ success: true, message: "Message sent to superadmin." });
};

const acknowledge = async (req, res) => {
    const { user: admin } = await auth.getUserAndGroup(
        auth.getToken(req.headers)
    );
    if (!req.body?.email) {
        return res
            .status(400)
            .send({ success: false, msg: "Bad request (admin)." });
    }
    const user = await User.findOne({ email: req.body.email });
    if (!user || user.account_type !== "CONFIRMED") {
        return res
            .status(400)
            .send({ success: false, msg: "Bad request (user)." });
    }
    ...
    const isAcknowledged =
        req.body.acknowledged === true || req.body.acknowledged === "true";
    user.account_type = isAcknowledged ? "ACKNOWLEDGED" : "REJECTED";
    await user.save();
    return res.status(200).send({
        success: true,
        msg: isAcknowledged
            ? "Elfogadtad a felhasználó csatlakozási kérését."
            : "A felhasználó kérését visszautasítottad.",
    });
};
...
const deleteUser = async (req, res) => {
    ...
    if (!userId) {
        return res
            .status(400)
            .send({ success: false, msg: "No user ID provided." });
    }
    const { user: requirer, group: requirerGroup } = await auth.getUserAndGroup(
        auth.getToken(req.headers)
    );
    ...
    const isAdmin =
        requirerGroup.admin && requirerGroup.admin.equals(requirer._id);
        ...
        if (groupMembers.length > 1) {
            return res
                ...
                .send({
                    success: false,
                    msg:
                        "A profil törlése nem engedélyezett amíg más felhasználók is vannak a csoportban.",
                });
        }
    ...
    return res
        .status(403)
        .send({ success: false, msg: "Authorization failed." });
};
...
const getAdminData = async (req, res) => {
    ...
    const nonAdminUsers = groupMembers
        .filter(
            (m) =>
                !m._id.equals(group.admin) && m.account_type === "ACKNOWLEDGED"
        )
